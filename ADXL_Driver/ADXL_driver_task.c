...
ADXL_status_t ADXL_Task_Init(ADXL_Init_t *init_data)
{
	ADXL_status_t ret_val = ADXL_FAILURE;
	BaseType_t task_result;
	if(ADXL_RegInitAlternative(init_data) == ADXL_SUCCESS)
	{
...
void ADXL_SetEvent(ADXL_FSM_Events evt)
{
	...
	if(IsIsr())
	{
		...
		xQueueSendFromISR(ADXL_EvtQueue, &user_event, &xHigherPriorityTaskWoken);
